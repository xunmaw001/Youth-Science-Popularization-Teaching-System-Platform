...
<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"rgba(224, 224, 224, 1)"}'>
    <view
        ...
        <view style="display: flex;justify-content: space-between;">
            <text v-if="isAuth('kechuangzuopin','修改')" class="cuIcon-edit"
                  @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="isAuth('kechuangzuopin','删除')" class="cuIcon-delete"
            ...
            v-if="isAuth('kechuangzuopin','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = await this.$api.list(`kechuangzuopin`, params);
			...
			async search() {
                ...
                let res = await this.$api.list(`kechuangzuopin`, searchForm);
				// 如果是第一页数据置空
