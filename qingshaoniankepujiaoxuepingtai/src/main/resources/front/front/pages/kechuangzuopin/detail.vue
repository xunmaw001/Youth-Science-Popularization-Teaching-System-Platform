...
<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback"
		...
		<view class="container">
			...
			<!-- 名字-->
						<view :style='{"textAlign":"left","fontSize":"45upx","color":"black"}'>{{detail.kechuangzuopinName}}
						</view>
				</view>
   	 <view style="margin-top: 10rpx;"></view>
					...
					<view class="detail-list-item"
						...
						<view class="text" :style='{"color":"#47EDB5","textAlign":"center","fontSize":"24rpx"}'>
                            {{detail.kechuangzuopinValue}}</view>
					</view>
			...
			<view class="time-content"
				...
				<view class="content" style="line-height: 50upx;letter-spacing: 5upx;"
					...
					<rich-text :nodes="detail.kechuangzuopinContent"></rich-text>
				</view>
			</view>
...
<script>
	export default {
		...
		methods: {
			...
			// 获取详情
			async init() {
				let res = await this.$api.info('kechuangzuopin', this.id);
				this.detail = res.data;
				// 主图
				this.swiperList = this.detail.kechuangzuopinPhoto ? this.detail.kechuangzuopinPhoto.split(",") : [];
			},
			...
			async upCallback(mescroll) {
				let res = await this.$api.list('kechuangzuopinLiuyan', {
					page: mescroll.num,
					limit: mescroll.size,
					kechuangzuopinId: this.id
				});
				// 如果是第一页数据置空
			...
			// 添加评论
			async onCommentTap() {
                this.$utils.jump(`../kechuangzuopinLiuyan/add-or-update?kechuangzuopinId=${this.id}`)
            },
		}
	}
</script>
