        ...
        <form class="app-update-pv">
			<view
                ...
                <textarea
                        ...
                        v-model='ruleForm.wenzhangLiuyanText' placeholder="评论内容"></textarea>
            </view>
    ...
    export default {
        ...
        async onLoad(options) {
            ...
            if (options.id) {
                ...
                // 获取信息
                let res = await this.$api.info(`wenzhangLiuyan`, this.ruleForm.id);
                this.ruleForm = res.data;
            }
            if(options.wenzhangId){
                this.ruleForm.wenzhangId = options.wenzhangId;
            }
            // 跨表
            // this.styleChange()
        },
        methods: {
            ...
            async onSubmitTap() {
			if ((!this.ruleForm.wenzhangLiuyanText)) {
                this.$utils.msg(`评论内容不能为空`);
                return
            }
                if (this.ruleForm.id) {
                    await this.$api.update(`wenzhangLiuyan`, this.ruleForm);
                } else {
                    await this.$api.save(`wenzhangLiuyan`, this.ruleForm);
                }
