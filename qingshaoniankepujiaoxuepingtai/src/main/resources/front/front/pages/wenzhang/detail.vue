...
<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback"
		...
		<view class="container">
			...
			<!-- 名字-->
						<view :style='{"textAlign":"left","fontSize":"45upx","color":"black"}'>{{detail.wenzhangName}}
						</view>
   	 ...
   	 <view style="margin-top: 10rpx;"></view>
					...
					<view class="detail-list-item"
						...
						<view class="lable"
							  :style='{"width":"auto","fontSize":"28rpx","color":"#47EDB5","textAlign":"left"}'>
							文章类型：</view>
						<view class="text" :style='{"color":"#47EDB5","textAlign":"center","fontSize":"24rpx"}'>
                            {{detail.wenzhangValue}}</view>
					</view>

                    <view class="detail-list-item"
                          :style='{"boxShadow":"0 0 4rpx 0 #006633 insta","margin":"0 60rpx 0 60rpx","borderColor":"rgba(218, 218, 218, 1)","backgroundColor":"#fff","borderRadius":"0","borderWidth":"2rpx","borderStyle":"none none solid none ","height":"68rpx"}'
                          @tap="download(detail.wenzhangFile)">
                        <view class="lable"
                              :style='{"width":"auto","fontSize":"28rpx","color":"#47EDB5","textAlign":"left"}'>
							文章：</view>
                        <view class="text" :style='{"color":"#47EDB5","textAlign":"center","fontSize":"24rpx"}'>
			...
			<view class="time-content"
				...
				<view class="content" style="line-height: 50upx;letter-spacing: 5upx;"
					...
					<rich-text :nodes="detail.wenzhangContent"></rich-text>
				</view>
			</view>
			...
			<view class="time-content"
				...
				<view class="comoment-header"
					...
					<view
                        ...
                        <view class="cuIcon-add">添加评论</view>
                    </view>
				</view>
...
<script>
	export default {
		...
		methods: {
			...
			// 获取详情
			async init() {
				let res = await this.$api.info('wenzhang', this.id);
				this.detail = res.data;
				// 主图
				this.swiperList = this.detail.wenzhangPhoto ? this.detail.wenzhangPhoto.split(",") : [];
			},
			// 收藏
			async getwenzhangCollection() {
                let params = {
                    page: 1,
                    limit: 1,
                    wenzhangId: this.id,
                ...
                let res = await this.$api.list(`wenzhangCollection`, params);
			...
			async shoucang() {
                ...
                let params = {
                    page: 1,
                    limit: 1,
                    wenzhangId: _this.detail.id,
                    yonghuId: _this.user.id,
                    wenzhangCollectionTypes:'1'
                }
                let res = await _this.$api.list(`wenzhangCollection`, params);
                if (res.data.list.length == 1) {
                    let wenzhangCollectionId = res.data.list[0].id;
                    uni.showModal({
                        title: '提示',
                        content: '是否取消',
                        success: async function(res) {
                            if (res.confirm) {
                                await _this.$api.del('wenzhangCollection', JSON.stringify([wenzhangCollectionId]));
                                _this.$utils.msg('取消成功');
                                _this.getwenzhangCollection();
                            }
                        }
                    });
                    return;
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            await _this.$api.add('wenzhangCollection', {
                                yonghuId: _this.user.id,
                                wenzhangId: _this.detail.id,
                                wenzhangCollectionTypes:1
                            });
			...
			async upCallback(mescroll) {
				let res = await this.$api.list('wenzhangLiuyan', {
					page: mescroll.num,
					limit: mescroll.size,
					wenzhangId: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
                this.$utils.jump(`../wenzhangLiuyan/add-or-update?wenzhangId=${this.id}`)
            },
			// 获取赞踩
			async getThumbsup() {
				let _this = this;
				...
				if (res.data.list.length > 0) {
					if (res.data.list[0].wenzhangCollectionTypes == '2') {
						_this.thumbsupFlag = true;
					}
					if (res.data.list[0].wenzhangCollectionTypes == '3') {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								...
								await _this.$api.update('wenzhang', _this.detail);
								_this.$utils.msg('取消成功');
