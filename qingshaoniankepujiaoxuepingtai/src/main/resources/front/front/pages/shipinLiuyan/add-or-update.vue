        ...
        <form class="app-update-pv">
			<view
                ...
                <textarea
                        ...
                        v-model='ruleForm.shipinLiuyanText' placeholder="评论内容"></textarea>
            </view>
    ...
    export default {
        ...
        async onLoad(options) {
            ...
            if (options.id) {
                ...
                // 获取信息
                let res = await this.$api.info(`shipinLiuyan`, this.ruleForm.id);
                this.ruleForm = res.data;
            }
            if(options.shipinId){
                this.ruleForm.shipinId = options.shipinId;
            }
            // 跨表
            // this.styleChange()
        },
        methods: {
            ...
            async onSubmitTap() {
			if ((!this.ruleForm.shipinLiuyanText)) {
                this.$utils.msg(`评论内容不能为空`);
                return
            }
                if (this.ruleForm.id) {
                    await this.$api.update(`shipinLiuyan`, this.ruleForm);
                } else {
                    await this.$api.save(`shipinLiuyan`, this.ruleForm);
                }
