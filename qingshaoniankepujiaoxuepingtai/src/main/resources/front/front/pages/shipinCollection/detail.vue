   	 ...
   	 <view style="margin-top: 10rpx;"></view>
					...
					<view class="detail-list-item"
						...
						<view class="text" :style='{"color":"#47EDB5","textAlign":"center","fontSize":"24rpx"}'>
                            {{detail.shipinCollectionValue}}</view>
					</view>
...
<script>
	export default {
		...
		methods: {
			...
			// 获取详情
			async init() {
				let res = await this.$api.info('shipinCollection', this.id);
				this.detail = res.data;
				// 主图
				this.swiperList = this.detail.shipinCollectionPhoto ? this.detail.shipinCollectionPhoto.split(",") : [];
			},
